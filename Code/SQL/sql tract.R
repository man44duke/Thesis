...
#######################################
queries <- c()
queries <- c(queries, "Select  census_tract, AVG(income) as average_income", "Select  census_tract, ((count(*) filter (where action_taken = 3))::decimal  / count(*)) as percent_denied")
queries <- c(queries,  "Select  census_tract, AVG(loan_amount) as loan_amount")
queries <- c(queries, "Select  census_tract, AVG(interest_rate::decimal) filter (where interest_rate != 'Exempt') as interest_rate", "Select  census_tract, AVG(rate_spread::decimal) filter (where rate_spread != 'Exempt') as rate_spread")
queries <- c(queries,"Select  census_tract, ((count(*) filter (where hoepa_status = 1))::decimal  / count(*)) as hoepa_status", "Select  census_tract, AVG(total_loan_costs::decimal) filter (where total_loan_costs != 'Exempt') as total_loan_costs" )
queries <- c(queries, "Select  census_tract, AVG(total_points_and_fees::decimal) filter (where total_points_and_fees != 'Exempt') as total_points_and_fees", "Select  census_tract, AVG(origination_charges::decimal) filter (where origination_charges != 'Exempt') as origination_charges")
queries <- c(queries, "Select  census_tract, AVG(discount_points::decimal) filter (where discount_points != 'Exempt' and discount_points != '') as discount_points", "Select  census_tract, AVG(lender_credits::decimal) filter (where lender_credits != 'Exempt' and lender_credits != '') as lender_credits")
queries <- c(queries, "Select  census_tract, AVG(loan_term::decimal) filter (where loan_term != 'Exempt' and loan_term != '') as loan_term" )
...
queries <- c(queries, "Select  census_tract, ((count(*) filter (where loan_type = 1))::decimal  / count(*)) as loan_conventional", "Select  census_tract, AVG(property_value::decimal) filter (where property_value != 'Exempt' and property_value != '') as property_value") 
...
queries <- c(queries, "Select  census_tract, AVG(tract_minority_population_percent::decimal) as tract_minority_population_percent", "Select  census_tract, AVG(ffiec_msa_md_median_family_income::decimal) as msa_median_income", "Select  census_tract, AVG(tract_to_msa_income_percentage::decimal) as tract_msa_income_percentage")
#########################################
...
for(q in queries){
  query.total <- paste(q, " 
                From home
                Where total_units = '1' and income <= 490 and loan_amount <= 1035000 and not loan_term = '360' 
...
query.marketShare <- paste(
  "Select home.census_tract, ((count(*) filter (where home.lei = modes.mode_lender))::decimal  / count(*)) as market_share
   FROM home, modes
   Where home.census_tract = modes.census_tract and not loan_term = '360'
...
query.ltv <- paste(
  "Select  census_tract, AVG(loan_to_value_ratio::decimal) filter (where loan_to_value_ratio != 'Exempt') as loan_to_value_ratio
   FROM home
   Where total_units = '1' and income <= 490 and loan_amount <= 1035000 and not loan_term = '360' and loan_to_value_ratio::decimal <= 100 and loan_to_value_ratio != 'Exempt'
   Group by census_tract;", sep = "")
